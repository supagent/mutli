...
func runUpdate(_ *cobra.Command, _ []string) error {
	...
	if cli.IsBrewInstall() {
		...
		if err != nil {
			...
			return fmt.Errorf("brew upgrade failed: %w\nYou can try manually: brew upgrade supagent/multi-cli/multica", err)
		}
	...
	if latest == nil {
		return fmt.Errorf("could not determine latest version; try: multica update (requires gh auth login for private repos)")
	}
