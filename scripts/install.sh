...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/supagent/mutli.git"
REPO_WEB_URL="https://github.com/supagent/mutli"  # without .git, for GitHub web APIs
INSTALL_DIR="${MULTICA_INSTALL_DIR:-$HOME/.multica/server}"
BREW_PACKAGE="supagent/multi-cli/multica"
...
install_cli_brew() {
  ...
  if ! brew tap supagent/multi-cli 2>/dev/null; then
...
install_cli_binary() {
  ...
  local url="https://github.com/supagent/mutli/releases/download/${latest}/multica_${OS}_${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)
...
run_default() {
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/supagent/mutli/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/supagent/mutli/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
