...
将编码和研究 Agent 变成真正的队友——分配任务、跟踪进度、积累技能。
...
Multica 将 AI Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、研究课题、报告阻塞问题、更新状态。

不再需要复制粘贴 prompt，不再需要盯着运行过程。你的 Agent 出现在看板上、参与对话、随着时间积累可复用的技能。可以理解为开源的 Managed Agents 基础设施——厂商中立、可自部署、专为人类 + AI 团队设计。支持 **Claude Code**、**Codex**、**OpenClaw**、**OpenCode** 和 **OpenHarness**。
...
daemon 在后台运行，保持你的机器与 Multica 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`openclaw`、`opencode`、`oh`）。
...
进入 **设置 → Agents**，点击 **新建 Agent**。选择你刚连接的 Runtime，选择 Provider（Claude Code、Codex、OpenClaw、OpenCode 或 OpenHarness），并为 Agent 起个名字——它将以这个名字出现在看板、评论和任务分配中。
...
| Agent 运行时 | 本地 daemon 执行 Claude Code、Codex、OpenClaw、OpenCode、Hermes 或 OpenHarness |
