...
3. It spawns the appropriate agent CLI (Claude Code, Codex, OpenClaw, OpenCode, Hermes, or OpenHarness)
...
| Provider | CLI Command | Description |
|----------|-------------|-------------|
| OpenHarness | `oh` | Research agent — web search, page reading, report writing |

The daemon auto-detects which CLIs are available on your PATH and registers them as available runtimes.

> **Free quick start:** Install OpenHarness (`pip install openharness-ai`) and ModelRelay (`npx -y modelrelay`) for a capable research agent at $0/task. See the [CLI and Daemon Guide](/docs/guides/cli-daemon) for details.
