...
```
Fetch https://github.com/supagent/mutli/blob/main/CLI_INSTALL.md and follow the instructions to install Multica CLI, log in, and start the daemon on this machine.
```
...
```bash
brew tap supagent/multi-cli && brew install multica
```
...
LATEST=$(curl -sI https://github.com/supagent/mutli/releases/latest | grep -i '^location:' | sed 's/.*tag\///' | tr -d '\r\n')

# Download and extract
curl -sL "https://github.com/supagent/mutli/releases/download/${LATEST}/multica_${OS}_${ARCH}.tar.gz" -o /tmp/multica.tar.gz
...
- If you use Scoop, the installer will use it automatically: `scoop bucket add multica https://github.com/supagent/scoop-bucket.git && scoop install multica`
